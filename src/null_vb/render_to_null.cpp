...
int main( int argc, const char *argv[] ) {
  ...
  while( true ) {
    ...
    wait_for_executed( device, queue, command_buffer );
    {
      std::uint32_t center = reinterpret_cast< std::uint32_t* >( raw_image_buffer.get() )[ 120 * width + 160 ];
      std::uint8_t r = ( center >> 16 ) & 0xFF;
      std::uint8_t g = ( center >> 8 ) & 0xFF;
      std::uint8_t b = center & 0xFF;
