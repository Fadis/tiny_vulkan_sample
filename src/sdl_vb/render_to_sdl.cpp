...
#include <tiny_vulkan_sample/sdl_utils.hpp>
...
int main( int argc, const char *argv[] ) {
  ...
  {
    {
      auto mem = texture.lock< std::uint32_t >();
      for( int y = 0; y != 240; ++y ) {
        for( int x = 0; x != 320; ++x ) {
          mem.get()[ x + y * 320 ] = 0xFF;
        }
      }
    }
    renderer.clear();
  ...
  const std::array< vk::DescriptorPoolSize, 1u > descriptor_pool_size{
  ...
  const std::array< vk::DescriptorSetLayoutBinding, 1u > descriptor_set_layout_binding{
  ...
  while( !exit_flag ) {
    ...
    t.end_rendering();
    {
      auto mem = texture.lock< void >();
      std::memcpy( mem.get(), raw_image_buffer.get(), width * height * sizeof( std::uint32_t ) );
    }
    renderer.clear();
    renderer.draw( texture );
    renderer.present();
